...
export default {
  setup() {
    ...
    return {
      designSteel, loadAnimation, lottieScroll, animateTo, animateFromTo,
    };
  },
  mounted() {
    const stealAnimation = this.loadAnimation('.steel-anim', steel);
    this.lottieScroll(stealAnimation, { start: 20, end: 60, to: 100 });
    this.animateFromTo('.design-steel-text-wrap--left .ix-shadow', { y: '-50%' }, { y: '50%' }, 20, 35);
    ...
    this.animateFromTo('.design-steel-text-wrap--right .ix-shadow', { y: '-50%' }, { y: '50%' }, 35, 50);
